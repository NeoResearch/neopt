...
// internal includes
#include<services/ConsoleServiceBase.h>

// core includes
//#include<core/wallets/Wallet.h>
#include<ledger2/plugin/PluginSystem.h>
#include<ledger2/plugin/IPlugin.h>

namespace neopt
{

class MainService : public ConsoleServiceBase
{
public:
...
public:
   ...
   MainService(PluginSystem& _pluginSystem) :
      pluginSystem(_pluginSystem), ConsoleServiceBase()
   {
   }
...
*/
protected:
   bool OnCommand(const vector<string>& args)
   {
      if (pluginSystem.SendMessage(args))
         return true;

      string cmd = khelper::ToLower(args[0]);
      if(cmd == "broadcast")
         return false; //OnBroadcastCommand(args);
      else if(cmd == "relay")
         return false; //OnRelayCommand(args);
      else if(cmd == "sign")
         return false; //OnSignCommand(args);
      else if(cmd == "change")
         return false; //OnChangeCommand(args);
      else if(cmd == "create")
         return false; //OnCreateCommand(args);
      else if(cmd == "export")
         ...
         return false; //return OnListCommand(args);
      else if(cmd == "claim")
         return false; //OnClaimCommand(args);
      else if(cmd == "open")
         return false; //OnOpenCommand(args);
      else if(cmd == "close")
         return false; //OnCloseCommand(args);
      else if(cmd == "rebuild")
         return false; //OnRebuildCommand(args);
      else if(cmd == "send")
         return false; //OnSendCommand(args);
      else if(cmd == "show")
         return false; //OnShowCommand(args);
      else if(cmd == "start")
         return false; //OnStartCommand(args);
      else if(cmd == "upgrade")
         return false; //OnUpgradeCommand(args);
      else if(cmd == "deploy")
         return false; //OnDeployCommand(args);
      else if(cmd == "invoke")
         return false; //OnInvokeCommand(args);
      else if(cmd == "install")
         return false; //OnInstallCommand(args);
      else if(cmd == "uninstall")
         return false; //OnUnInstallCommand(args);
      else
         return ConsoleServiceBase::OnCommand(args);
   }
...
/*
   ...
   var tx = LoadScriptTransaction(
        ...
        args[2], //  paramTypes
        args[3], // /* returnType
        args[4].ToBool(),// /* hasStorage
        args[5].ToBool(), // /* hasDynamicInvoke
        args[6].ToBool(), // /* isPayable
        args[7], // /* contractName
        args[8], // /* contractVersion
        args[9], // /* contractAuthor
        args[10], // /* contractEmail
        args[11], // /* contractDescription
        out var scriptHash); // /* scriptHash
...
protected:
   virtual void OnStart(const vector<string>& args)
   {
